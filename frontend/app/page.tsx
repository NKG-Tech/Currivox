...
// Pour les liens directs (ex: /metrics), on lit l’URL publique Render
...
export default function Home() {
  ...
  // --- API calls via proxy /api/* (cf. next.config.mjs rewrites)
  ...
  async function handleImport(file) {
    ...
    try {
      ...
      // Ta route actuelle (dépend de extract.py)
      let res = await fetch("/api/resume/upload", { method: "POST", body: form });

      // Fallback si non dispo (501/404) → à activer si tu ajoutes /ingest/pdf côté backend
  ...
  async function doAnalyze() {
    ...
    setAnalysis(null);
    try {
      const payload = { resume, job, language: "fr", gender: "auto" };
  ...
  async function exportPdf() {
    ...
    try {
      ...
      // Téléchargement du PDF renvoyé par l’API
  ...
  return (
    <main style={{ maxWidth: 1100, margin: "32px auto", padding: 16 }}>
      ...
      {errorMsg && (
        <div style={{
          background: "#fde2e1", border: "1px solid #f6b1ad",
          padding: 12, borderRadius: 10, color: "#7a1d1a", marginBottom: 16
        }}>
          {errorMsg}
        </div>
      )}
